...
(function() { 
    ...
    GeoUtils.isPointOnPolyline = function(point, polyline, map){
        //检查类型
        if(!(point instanceof BMap.Point) ||
        ...
        var touchPixel = map.pointToOverlayPixel(point);

        var pts = polyline.getPath();
        for(var i = 0; i < pts.length - 1; i++){
            var curPt = pts[i];
            var curPixel = map.pointToOverlayPixel(curPt);
            var nextPt = pts[i + 1];
            var nextPixel = map.pointToOverlayPixel(nextPt);

            if(nextPixel.x==curPixel.x){
                var x = curPixel.x;
                var y = touchPixel.y;
            }else if(nextPixel.y==curPixel.y){
                var x = touchPixel.x;
                var y = curPixel.y;
            }else{
                var k1 = (nextPixel.y-curPixel.y)/(nextPixel.x-curPixel.x); //斜率k1
                var k2 = -1/k1; //斜率k2
                var x = (touchPixel.y-curPixel.y-k2*touchPixel.x+k1*curPixel.x)/(k1-k2);
                var y = curPixel.y+k1*(x-curPixel.x);
            }

            var high = Math.sqrt(Math.pow(touchPixel.x-x,2)+Math.pow(touchPixel.y-y,2));
            

            if(x >= Math.min(curPixel.x,nextPixel.x) && x <= Math.max(curPixel.x,nextPixel.x) && 
                    y>=Math.min(curPixel.y,nextPixel.y) && y<=Math.max(curPixel.y,nextPixel.y) && high<=8)
            {
                console.log(high);
                var p = new BMap.Pixel(x,y);
                var marker = new BMap.Marker(map.overlayPixelToPoint(p)); //起点，不在ajax中定义
            ...
            //     console.log(mapZoom);
            //     console.log(judgeprecision);
            //     console.log(precision);
            //     if(precision < judgeprecision && precision > -judgeprecision){//实质判断是否接近0
     ...
     */
    GeoUtils.isPointOnPolyline1 = function(point, polyline, mapZoom){
        //检查类型
        if(!(point instanceof BMap.Point) ||
            !(polyline instanceof BMap.Polyline)){
            return false;
        ...
        if(mapZoom==13){
            var judgeprecision = 1e-5;
        }else if(mapZoom>13&&mapZoom<18){
            var judgeprecision = 1e-6;
        }else if(mapZoom==18){
            var judgeprecision = 1e-7;
        }else if(mapZoom==19){
            var judgeprecision = 5e-8
        }
        else{
            var judgeprecision = 1e-4;
        }
